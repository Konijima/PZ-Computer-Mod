...
require("Computer/Manager/InventoryTooltipManager")

local function getCleanString(floatVal)
...
local function customDoTooltip(tooltip, fields, item)
    ...
    elseif item:isEquipped() then
        cleanString = getCleanString(item:getEquippedWeight())
        weightItem:setValue(cleanString .. "    (" .. getCleanString(item:getUnequippedWeight()) .. " " .. getText("Tooltip_item_Unequipped") .. ")", 1.0, 1.0, 1.0, 1.0)
    elseif item:getAttachedSlot() > -1 then
        cleanString = getCleanString(item:getHotbarEquippedWeight())
        weightItem:setValue(cleanString .. "    (" .. getCleanString(item:getUnequippedWeight()) .. " " .. getText("Tooltip_item_Unequipped") .. ")", 1.0, 1.0, 1.0, 1.0)
    else
        cleanString = getCleanString(item:getUnequippedWeight())
        weightItem:setValue(cleanString .. "    (" .. getCleanString(item:getUnequippedWeight()) .. " " .. getText("Tooltip_item_Equipped") .. ")", 1.0, 1.0, 1.0, 1.0)
    end
    ...
    for _, field in pairs(fields) do
        if field.result then
            ...
            elseif field.fieldType == "label" then
                local labelColor = getSafeColor(field.result.labelColor, { r=1, g=1, b=1, a=1 })
            ...
            elseif field.fieldType == "field" then
                local labelColor = getSafeColor(field.result.labelColor, { r=1, g=1, b=0.8, a=1 })
                local color = getSafeColor(field.result.color, { r=1, g=1, b=1, a=1 })
            ...
            elseif field.fieldType == "progress" and type(field.result.value) == "number" then
                local labelColor = getSafeColor(field.result.labelColor, { r=1, g=1, b=0.8, a=1 })
                local color = getSafeColor(field.result.color, { r=0, g=0.6, b=0, a=0.7 })
                layoutItem:setLabel(field.name..":", labelColor.r, labelColor.g, labelColor.b, labelColor.a)
...
function ISToolTipInv:render()
    ...
    local inventoryTooltip = InventoryTooltipManager.Tooltips[self.item:getFullType()]
