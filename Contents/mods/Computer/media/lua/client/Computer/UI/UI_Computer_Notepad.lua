UI_Computer_Notepad = ISCollapsableWindow:derive("UI_Computer_Notepad");


--************************************************************************--
--** UI_Computer_Notepad:initialise
--**
--************************************************************************--

function UI_Computer_Notepad:initialise()
...
--    self.winwo:setY(self.y);
    ...
    self.yes = ISButton:new((self:getWidth() / 2) - btnWid - 5, self:getHeight() - padBottom - btnHgt, btnWid, btnHgt, getText("UI_Ok"), self, UI_Computer_Notepad.onClick);
    ...
    self.no = ISButton:new((self:getWidth() / 2) + 5, self:getHeight() - padBottom - btnHgt, btnWid, btnHgt, getText("UI_Cancel"), self, UI_Computer_Notepad.onClick);
    ...
    if not self.locked then
        self.deleteButton = ISButton:new(self.entry.x, 50 + height, 15, 13, "", self, UI_Computer_Notepad.onClick);
        ...
        self.lockButton = ISButton:new(self.deleteButton.x + self.deleteButton.width + 3, self.deleteButton.y, 15, 13, "", self, UI_Computer_Notepad.onClick);
    ...
    if self.numberOfPages > 1 then
        ...
        self.nextPage = ISButton:new(self.width / 2 + 3, self.pageLabel.y + 3, 15, 15, ">", self, UI_Computer_Notepad.onClick);
        ...
        self.previousPage = ISButton:new(self.nextPage.x - 20, self.nextPage.y, 15, 15, "<", self, UI_Computer_Notepad.onClick);
...
function UI_Computer_Notepad:destroy()
...
function UI_Computer_Notepad:onClick(button)
...
--        print("set text from pos " .. self.currentPage .. " text " .. self.newPage[self.currentPage]);
    ...
    elseif button.internal == "LOCKBOOK" then
        self.lockButton:setImage(getTexture("media/ui/lock.png"));
        self.lockButton.internal = "UNLOCKBOOK";
        self.disk:setLockedBy(self.character:getUsername());
        self.title:setEditable(false);
        self.entry:setEditable(false);
...
function UI_Computer_Notepad:prerender()
    self.pinButton:setVisible(false);
...
--************************************************************************--
--** UI_Computer_Notepad:render
--**
--************************************************************************--
function UI_Computer_Notepad:render()

end

function UI_Computer_Notepad:onGainJoypadFocus(joypadData)
...
function UI_Computer_Notepad:onJoypadDown(button)
...
--************************************************************************--
--** UI_Computer_Notepad:new
--**
--************************************************************************--
function UI_Computer_Notepad:new(x, y, width, height, target, onclick, character, disk, defaultEntryText, title, lineNumber, editable, numberOfPages)
    local o = {}
    ...
    for i=0,disk:getCustomPages():size() - 1 do
        o.newPage[i + 1] = disk:seePage(i + 1);
    end
    o.locked = not editable;
    if disk:getLockedBy() and disk:getLockedBy() ~= character:getUsername() and not isAdmin() then
        o.locked = true;
        o.numberOfPages = disk:getCustomPages():size();
    end
    return o;
end
